 ...
 *
 *   BASE=http://127.0.0.1:3911 CPU=6 [RETOUR=1] [SANS_CV=1] node scripts/gel-chapitre.mjs [routes…]
 *   RETOUR=1 mesure aussi la flèche retour vers le chapitre déjà vu ; SANS_CV=1
 *   rejoue la règle display:none d'avant le 2026-09-06 (témoin, même build).
...
for (const r of routes) {
  ...
  // SANS_CV=1 : rejouer la règle d'avant (display:none) sur le même build — le témoin de la comparaison
  if (process.env.SANS_CV === "1") await p.addStyleTag({ content: ".chapter-view[hidden]{display:none !important;content-visibility:visible !important}" });
  if (css400) {
  ...
  // RETOUR : ArrowLeft vers le chapitre déjà vu — ce que coûte une revisite
  let retourApres = null, retourMax = 0;
  if (process.env.RETOUR !== "0") {
    await p.evaluate(() => { window.__lt = []; });
    const t1 = Date.now();
    await p.keyboard.press("ArrowLeft");
    for (let i = 0; i < 400; i++) {
      const actif = await p.evaluate(() => document.querySelector('[data-chapter-active="true"]')?.getAttribute("data-chapter-index") ?? "1");
      if (actif === "0") { retourApres = Date.now() - t1; break; }
      await p.waitForTimeout(25);
    }
    let l2 = -1, q2 = 0;
    while (q2 < 4) { await p.waitForTimeout(500); const n = await p.evaluate(() => window.__lt.length); if (n === l2) q2++; else { q2 = 0; l2 = n; } }
    retourMax = await p.evaluate(() => Math.round(Math.max(0, ...window.__lt.map(([, d]) => d))));
    console.log(`${"".padEnd(58)}    retour (ArrowLeft, chapitre déjà vu) : changé après ${(retourApres === null ? "—" : retourApres + " ms").padStart(8)} · tâche max ${String(retourMax).padStart(5)} ms`);
  }
  await ctx.close();
}
await nav.close();
