 ...
 *     this pass).
 *
 * This is the CLIENT half of /connexion (it reads useAuth() and drives
 * router navigation). The server half — `page.tsx` — owns the shell: the
 * page `metadata`, and the header manifests (notions + épreuves) that the
 * Notions panel and the ⌘K palette need. That split is what the earlier
 * version of this header called "a follow-up pass": while the page itself
 * was the client component, it could not call the server-only manifests,
 * so the header's Notions panel opened onto an empty 720 px box and ⌘K
 * found nothing — on the one page a lost student is most likely to land.
 *
 * Everything below is byte-identical to that version; only the <PageShell>
 * wrapper moved out, to `page.tsx`.
 *
