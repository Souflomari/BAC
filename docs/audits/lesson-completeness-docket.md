...
| — dont **catégorie A** (in-cadre, à intégrer) | **26** (9 « contenu neuf à écrire », 17 « pont léger » cross-notion vers un rung déjà existant ailleurs dans le corpus) |
...
| A5 | `equations-differentielles` | **Équation caractéristique générale** $ar^2+br+c=0$ pour $ay''+by'+cy=0$ (cas Δ>0 / Δ=0 / Δ<0) | 2022 q2a (bk-2022-n-x4, = sommet r-bac) | R4 ne couvre que $y''+\omega^2y=0$ (sans terme $y'$) ; `reasoning` autoportant, ré-explique la méthode de A à Z | Rung « R4-bis » — déjà recommandé tel quel par la SCOPE NOTE du fichier. Cadre `maths-sm.yaml:154+157` confirme sans ambiguïté le programme SM. **Voir aussi B1 ci-dessous** : le sujet qui porte cette lacune est tagué filière SExp dans la transcription, ce qui contredit la limite SExp dérivée du même cadre — l'appartenance filière exacte reste à trancher, pas la légitimité pédagogique du rung |
...
|---|---|---|---|---|---|
| A17 | `piles` | **Schéma conventionnel** (notation à barres : anode/cathode, pont salin) | 2020 q2 (+ sommet r-bac déjà concerné) | Écart déjà présent dans `exercises.yaml` r-bac lui-même | Extension de R3. **Cadre-confirmé explicitement** : `pc-physique-chimie.yaml:509` — « Schématiser une pile (schéma conventionnel) » |
...
| A23 | `ondes-mecaniques-progressives` (ou `-periodiques`) | Définition générale d'**onde transversale** (perturbation ⊥ propagation) | `-periodiques` 2019 q1, 2020 q2 | Seule la longitudinale est formellement définie (R4 de `-periodiques`) ; la transversale est déduite par contraste + rappel du chapitre précédent, qui ne la définit pas non plus formellement | Rung dédié, probablement en R1 de `ondes-mecaniques-progressives` (le chapitre chronologiquement premier) |
