...
| `web/scripts/zoom-sweep.mjs` | Le corpus avec le texte doublé (SC 1.4.4) : débord et texte COUPÉ **Attend `document.fonts.ready` avant de mesurer** (2026-09-05) : sous charge — trois navigateurs et deux serveurs sur la même machine —, `networkidle` arrivait avant les fontes du site et la largeur se lisait avec les métriques du SUBSTITUT, plus large : 61 débords de 47 à 200 px sur des pages qui, seules et à froid, mesurent 0. Une largeur qui dépend de l'instant où on la lit n'est pas une mesure. | Le zoom NAVIGATEUR (qui redimensionne tout, pas seulement le texte) |
