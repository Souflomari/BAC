...
| `web/scripts/dette-manipulable.mjs` | **CE QUE LE PRODUIT DOIT ENCORE À L'ÉLÈVE QUI MANIPULE.** La ligne dure de `.claude/CLAUDE.md` (décision ouverte n°4) : un asset généré ne remplace pas un manipulable là où la pédagogie exige la manipulation. Une spec qui prescrit `[[embed:slug]]` demande une chose que l'élève TOUCHE. **Les six substitutions du corpus sont toutes ÉCRITES**, chacune en tête du SVG qui remplace le manipulable, chacune nommant ce qui est perdu (« curseur Δt → ici, DEUX tailles de pas fixes ») — rien n'a été maquillé. Ce qui manquait est un endroit où les COMPTER : chaque dette vivait dans un en-tête que seul son lecteur voit, et `media-manipulable` (§11.129) n'en savait rien. Deux sens : une prescription ni livrée ni remplacée par écrit (franc, 0), et le nombre de substitutions (cliquet, 6). Mesuré : 4 prescriptions, 4 manipulables livrés, 6 substitutions, 0 promesse muette. Registre : `docs/audits/dette-manipulable-2026-09-20.md`. Essais rouges §11.135a/b. §11.135. **Au 2026-09-24 : 8 prescriptions, 1 substitution** (cliquet 1) — la cuve à ondes (scène 2D) et trois figures manipulables de pc/ (pH, Euler, chute avec frottement) en ont payé quatre ; une FIGURE MANIPULABLE (`.interactive.json` + module ENREGISTRÉ) compte désormais comme livraison. Et un angle mort refermé : le motif des prescriptions ne lisait que `[a-z0-9-]` — `[[embed:distribution-curseur-pH]]` et `[[embed:lecture-Ve-courbe-dosage]]` n'étaient comptées NULLE PART (ni livrées, ni muettes) : deux prescriptions invisibles, par la casse (§11.199). **Puis 0 substitution** (cliquet 0) : `lecture-Ve-courbe-dosage`, la dernière, payée à son tour par une figure manipulable — 8 prescriptions, 20 manipulables livrés, 0 promesse muette. Et un second angle mort de casse, vu par l'auteur de la figure : le nom exporté se dérivait du slug par `/-([a-z])/`, qui ne capitalise pas une lettre DÉJÀ capitale (« lecture-Ve-… » → « lecture-VeCourbeDosage », un nom qu'aucun module ne peut porter) ; corrigé ici, dans `dom-truth` et dans `figures-manipulables`, et la clé du registre est exigée aussi (§11.200) | Il lit des PRESCRIPTIONS, pas des besoins : une pédagogie qui exige la manipulation sans qu'aucune spec ne l'ait écrite lui est invisible, et les onze notions de SVT n'ont ni spec de ce genre ni embed. Il ne juge pas si le manipulable livré manipule la bonne grandeur |
...
- **la note « eau pâle » se montre pendant la course qui l'exige** (famille
  `eclairs`, §11.200) — la première, à 40 Hz, avant le verdict ; et à l'étape 1
  révélée, la liste des lectures ne contient que des lectures (famille
  `ergonomie`). Sabotage « le panneau d'avant » (les notes DANS la liste) : ces
  deux mesures rougissent, et elles seules ;
...
## `web/scripts/scene-plan-complexe.mjs` — le plan dit-il EXACTEMENT ce que la transformation fait, et le dessine-t-il là où il le dit ?

**Porte du « plan complexe »** (maths/nombres-complexes-2, en tête de R5 ; spec
`docs/pipeline/propositions/maths-nombres-complexes-2-scene-plan.md` §11 ; HANDOFF
§11.209). Le huitième manipulable PLAN, le premier des mathématiques sans 3D ; ni temps ni
course. `next start` + Chromium, le panneau trouvé par
`[data-scene="plan-complexe-transformation"]`. `test-plan-complexe` (10 tests, en CI)
garde le modèle exact ($\mathbb{Q}(\sqrt3)[i]$).

**Deux mesures par nombre, et c'est la particularité de cette porte.** (a) l'égalité de
CHAÎNE contre les tables de la spec (§5.3, recopiées chez elle dans la forme que le produit
écrit : `\tfrac` sur les étiquettes du plan, `\dfrac` dans les lectures) ; (b) l'égalité
NUMÉRIQUE à $10^{-9}$ entre la chaîne LUE — évaluée par un petit lecteur de TeX propre à la
porte (nombres, $i$, $\pi$, `\sqrt`, `\dfrac`, parenthèses, produit implicite) — et le
calcul flottant refait depuis les crans de la spec. Une belle forme exacte qui ne vaut pas
le bon nombre rougit au (b) ; un décimal juste rougit au (a) et à N10. N1 à N10 aux **70
états** du mode `coefficient` (7 coefficients × 5 points × 2 centres) et aux **15** du mode
`enonce` : $z'$ sur son étiquette (depuis $a$ ET $b$), $|c|$, les deux distances et leur
rapport (« — » au point fixe), $\arg c$, l'ÉCART ramené dans $]-\pi;\pi]$ (la borne $-\pi$
écrite $+\pi$ à $c = -2$), l'argument de l'image ÉGAL à l'écart sur l'axe réel et DIFFÉRENT
ailleurs (N5 dans les deux sens), $\omega$ calculé deux fois (par $b/(1-a)$ et par la
résolution de $(1-a)\omega = b$ en réels), les deux écritures (factorisée, développée) lues
et évaluées, $(z'-\omega)/(z-\omega) = a$, aucun décimal ni degré dans les lectures.

**Les PIXELS, dans les deux sens, l'échelle LUE.** `isotropie` : les traits des
graduations lus dans la rangée sous l'axe réel et la colonne à gauche de l'axe imaginaire,
une régression par axe — px/unité égaux à 0,5 % —, les deux demi-cordes du cercle unité
($\sqrt3/2$ d'unité chacune), le cadre CARRÉ et la fenêtre $[-9;9]^2$ symétrique.
`quadrillage-opaque` (un nœud vaut une ligne à 2 niveaux près). `point-a-sa-place` (M et M′
cherchés comme des DISQUES là où la porte les place — barycentre sous-pixel du disque, que
son halo isole). `longueurs-au-rapport`, `colineaires` (le segment le plus court DESSUS,
lu sur son axe), `arc-entre-les-bonnes-directions` (les pixels d'accent de l'arc, leur
ouverture mesurée DEPUIS ΩM, la pointe du côté de ΩM′ — un arc tracé depuis l'axe réel
rougit), `point-fixe-immobile` et `une-seule-etiquette-au-point-fixe`,
`balayage-invariants` (au clavier puis à la souris : pendant, les cinq invariants
identiques au caractère près, l'argument de l'image « — », aucun chiffre sur M et M′, l'arc
à 30° entre ΩM et ΩM′ ; relâché, tout revient). Et `avant-pari` (aucun accent, aucune
lecture, aucun contrôle ; M′ PRÉSENT à l'encre à S3 et S5, absent ailleurs ; Ω absent de
tout — point, nom, description — à S5), `formule-graduee` (la table du §7.6 C, étape par
étape, sur le panneau ET sur tous les textes du descripteur), `fuite-inter-etapes`,
`frontiere` (quinze formes, une sonde chacune, et les grilles : radicaux de 2, 3, 5, 10,
dénominateurs de π de 2 à 12), `katex` (aucun LaTeX brut, aucune erreur, aucun ω/Ω hors
KaTeX), `etiquettes`, `etiquettes-pres` (au-delà de 14 px, un FILET doit relier
l'étiquette à son point — lu aux pixels) et `cadre` à 1 280 et 390 px et au grand texte,
`immobile`, `annonce`, `theme`, `console`, `ergonomie`.

**Premier passage : 135 rouges sur 628 mesures, une centaine dans la porte** —
`Object.keys` range d'abord les clés qui ressemblent à des entiers (« 2 », « 4 ») et la
table A se lisait dans le désordre ; le quadrillage compté comme de l'encre ; un point fixe
cherché en encre sous le carré d'accent de Ω. Le reste était le produit (HANDOFF §11.209).
**Puis VERTE trois fois (631 mesures, 28 familles), puis 632 avec N9 (le compte des
crans : 7, 5, 2, 4, et aucun curseur hors du balayage) ; essai rouge 27/27, 17 formes de
frontière vues.** En CI dans le job `telephone`.
...
manipulable (INTERACTIVE-FIGURE-SPEC) est faite de trois morceaux écrits à part :
un SVG qui porte des `id`, un `.interactive.json` qui LIE ces ids à des clés de
calcul, un module `.ts` qui calcule. `validate-content` vérifie que les clés
existent ; `dom-truth` porte cinq balayages écrits à la main, un par figure
pilote. **Une sixième figure n'était vue par rien** : un id mal orthographié ne
casse rien, il ne fait rien. Cette porte lit son inventaire dans `content/` — elle
prend celles d'aujourd'hui ET celles de demain.

**Ce qu'elle mesure** (next start, Chromium) : `verrou` — en mouvement normal, le
curseur n'existe qu'à la dernière étape ; en mouvement réduit, la figure est
révélée d'un coup et le curseur est là d'emblée (le contrat écrit de
StagedFigure) ; `liaisons` — à quatre positions (bornes, valeur initiale, une
valeur de biais), chaque cible liée porte exactement ce que le module calcule
(`d`, `cx/cy` ou `x/y`, texte ; lu sous mouvement réduit : des valeurs posées,
pas des interpolations GSAP) ; `lecture` — la ligne annoncée est le gabarit
rempli ; `cible` — le curseur mesure ≥ 44 px ; `physique` — pour les figures
qu'elle NOMME, une seconde voie écrite dans la porte recalcule depuis la lecture
(le pH : la fraction, la somme 100, jamais « 0 » ni « 100 » ; Euler : le schéma
de la leçon jusqu'à 0,30 s, un écart jamais nul ; la chute : τ = m/k,
v_ℓ = mg/k) ; `console`.

**Au 2026-09-24 : VERTE, 89 mesures, 6 familles, 8 figures.** Premier passage :
cinq « rouges » qui étaient la PORTE — un clic intercepté (clic programmé
désormais : le geste est l'affaire de dom-truth), puis une page en mouvement
réduit où StagedFigure n'a plus de transport (d'où les deux passes).

**Au 2026-09-24, soir : VERTE, 110 mesures, 6 familles, 9 figures** (§11.200). La
neuvième, `lecture-Ve-courbe-dosage`, a une seconde voie d'un genre neuf : elle ne
refait pas un calcul, elle suit le trait DESSINÉ dans le SVG rendu
(`getPointAtLength`, bissection sur x = 90 + 20·V) — le point mobile doit être sur
ce trait (écart mesuré : 0,00 px aux six positions), le pH lu doit être le sien, et
« équivalence » ne s'écrit qu'à V_E, jamais au point où pH = 7 (13,3 mL, visité
exprès : `EN_PLUS`). Trois sabotages du produit, chacun pris par `physique` SEULE —
les liaisons, qui comparent le rendu au module, restent vertes quand c'est le module
qui ment : « équivalence » écrit à pH ≈ 7 (rouge à 13,3 mL seulement) ; des droites
entre les ancres au lieu des Bézier (rouge à 9,3 et 13,3 mL, vert aux ancres, où les
deux coïncident).

**NE DIT RIEN DE :** la physique des cinq figures de maths (la famille les nomme
« câblage seulement », à chaque passage) ; le glisser sur la courbe (le curseur
seul) ; le téléphone.

    node scripts/figures-manipulables.mjs --porte        (lève son propre next start)
...
le plancher maison est 48) ; **les listes de lectures** (§11.200) : chaque `<dl>`
du panneau ne contient que des couples terme/valeur — la cuve rangeait ses notes au
milieu de la sienne. Le module exporte `listes()` : une porte dont les lectures
