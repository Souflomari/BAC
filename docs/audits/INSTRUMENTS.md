...
| `web/scripts/veille-hydratation.mjs` | La VEILLE D'HYDRATATION, mesurée : sur un réseau lent émulé (KBPS/RTT, processeur ×4), chaque seconde — le bandeau « Recharger » est-il visible, la ligne « se prépare… » aussi, les deux ENSEMBLE, et quand l'hydratation arrive ; `PERTE=1` bloque le plus gros morceau d'entrée et mesure le délai perte → bandeau (+0,3 s depuis le 2026-09-11 ; +8,3 s avant). HANDOFF §11.29 | Le vrai réseau ; un bandeau qui MENT (il dit « connexion faible » aussi quand le morceau est perdu pour une autre raison) |
