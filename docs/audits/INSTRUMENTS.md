...
| `web/scripts/cv-chapitre.mjs` | Expérience : coût de démasquer un chapitre caché par `hidden` contre `content-visibility:hidden`, 1re et 2e fois, ×6, cinq leçons. A trouvé : **−35 % la première fois, ×30 à ×100 moins cher à chaque visite suivante** (20 ms au lieu de 600–1 300) — HANDOFF §11.22, levier consigné, pas pris | Le produit lui-même (l'expérience manipule le DOM servi) ; l'impression, le lecteur d'écran et les balayages sous `content-visibility` — à rejouer si le levier est pris |
