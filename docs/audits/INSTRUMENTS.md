...
| `web/scripts/zoom-sweep.mjs` | Le corpus avec le texte doublé (SC 1.4.4) : débord et texte COUPÉ **Attend `document.fonts.ready` avant de mesurer** (2026-09-05) — une hygiène, PAS l'explication des 61 débords vus ce jour-là : re-lancé seul, machine à froid, il en rend toujours 61, et l'arbre d'HIER (6dfcb92, reconstruit dans un worktree, son propre code) en rend 61 aussi. Le « 227 → 0 » consigné hier n'était donc pas reproductible ; les 61 étaient réels (fil d'Ariane borné en `ch`, items flex et grille sans `min-w-0`, boutons inline-flex à largeur max-content) et sont corrigés à la source. Ma première lecture — « les métriques du substitut » — était une hypothèse écrite avant la contre-mesure ; elle est retirée ici. | Le zoom NAVIGATEUR (qui redimensionne tout, pas seulement le texte) |
