...
Statut: vérifié — agent-vérificateur-adversarial, 2026-08-06 : source re-fetchée indépendamment (element 109797 → course-438/upload-80918 re-dérivé, 4 pages) ; diff caractère-par-caractère conforme au scan p.3 ($g(x)=2\sqrt{x}-2-\ln x$ sur $]0,+\infty[$, $g'(x)=\frac{\sqrt{x}-1}{x}$, croissance sur $[1,+\infty[$, encadrement $0\le\ln x\le 2\sqrt{x}$ avec la parenthèse d'aide « remarquer que $2\sqrt{x}-2\le 2\sqrt{x}$ », encadrement $0\le\frac{(\ln x)^3}{x^2}\le\frac{8}{\sqrt{x}}$, primitive $G:x\mapsto x\left(-1+\frac{4}{3}\sqrt{x}-\ln x\right)$, bornes $\int_1^4$ vérifiées au zoom, minuscule d'origine « en déduire » conservée, barème imprimé 0,5 + 0,5 + 0,5 + 1 + 0,75 + 0,75 = 4 pts, conforme à la ligne « Limites, dérivabilité et calcul intégral — 4 points » de la table des composantes p.1) ; mathématiques re-dérivées : $g'(x)=\frac{1}{\sqrt{x}}-\frac{1}{x}=\frac{\sqrt{x}-1}{x}$, $g(1)=0$ donc $\ln x\le 2\sqrt{x}-2\le 2\sqrt{x}$ sur $[1,+\infty[$, cube et division par $x^2$ donnent bien $\frac{8}{\sqrt{x}}$ (limite 0 par encadrement), $G'(x)=-1+2\sqrt{x}-\ln x-1=g(x)$ et $\int_1^4 g=\frac{19}{3}-4\ln 4>0$ ; classement re-contrôlé sur le scan : Q1 = 2,5 pts sur 4 avec la question la plus lourde (Q1-d, 1 pt) portant sur la limite de croissances comparées, Q2 sans IPP ni aire — dominante « limites » exacte ; filière Sciences Expérimentales et code NS 22F confirmés sur l'en-tête du scan.
...
Statut: vérifié — agent-vérificateur-adversarial, 2026-08-06 : source re-fetchée indépendamment (element 127180 → course-438/upload-84139 re-dérivé, 4 pages) ; diff caractère-par-caractère conforme au scan p.2 (équation $e^{2x}-4e^x+3=0$, inéquation $e^{2x}-4e^x+3\le 0$, limite $\lim_{x\to 0}\frac{e^{2x}-4e^x+3}{e^{2x}-1}$, équation $e^{2x}+e^x+4x=0$ sur $[-1,0]$, absence de phrase d'introduction conforme au scan, barème imprimé 0,5 + 0,5 + 0,5 + 0,5 = 2 pts, conforme à la ligne « fonctions numériques — 2 points » de la table des composantes p.1 ; doublon d'étiquette « Exercice 2 » sur la page 2 re-constaté, l'exercice transcrit portant bien l'étiquette « Exercice 1 : (2 points ) ») ; mathématiques re-dérivées : $t=e^x$ donne $(t-1)(t-3)$ d'où $x=0$ et $x=\ln 3$, la limite est bien une forme indéterminée $\frac{0}{0}$ levée par $\frac{(e^x-1)(e^x-3)}{(e^x-1)(e^x+1)}\to -1$, et $x\mapsto e^{2x}+e^x+4x$ est continue avec $f(-1)\approx-3,50<0<2=f(0)$ — TVI applicable sur $[-1,0]$ ; classement re-contrôlé sur le scan : cœur limites + continuité = Q1-c et Q2, soit 1 pt sur 2, aucune dérivée dans l'exercice ; filière Sciences Expérimentales et code NS 22F confirmés sur l'en-tête du scan.
