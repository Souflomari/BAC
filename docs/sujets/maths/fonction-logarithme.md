...
Statut: vérifié — agent-vérificateur-adversarial, 2026-08-06 : source re-fetchée indépendamment (element/127180 → `course-438/upload-84139`, 4 pages re-dérivées, problème sur p. 3-4) ; diff caractère-par-caractère conforme au scan (énoncé, structure 1→8, bornes, exposants, notation « $e^{\frac32}\simeq 4.5$ » avec point décimal, « Etudier » sans accent en Q8-a/b) — somme des barèmes = 9 pts = ligne « Problème · Etude de fonctions numériques et calcul intégral · 9 points » des composantes (p. 1) et bandeau « Problème : (9 points) » (p. 3). Mathématiques re-dérivées indépendamment : $f'(x)=2\ln x$ ✔ (d'où décroissance sur $]0,1[$, croissance sur $]1,+\infty[$), minimum $f(1)=-2$ ✔ qui donne exactement l'inégalité de Q6-b ($2x\ln x-2x\ge-2 \Leftrightarrow \ln x\ge\frac{x-1}{x}$), $f(x)=0\Leftrightarrow x=e$ et $f(x)=x\Leftrightarrow x=e^{3/2}\simeq4{,}48$ ✔ (ce qui justifie l'indication « on prend $e^{\frac32}\simeq 4.5$ » de Q4-b), IPP re-calculée : $\int_1^e x\ln x\,dx=\big[\tfrac{x^2}{2}\ln x\big]_1^e-\int_1^e\tfrac{x}{2}dx=\tfrac{1+e^2}{4}$ ✔, d'où $\int_1^e f=\tfrac{3-e^2}{2}$, et $J=[-2,+\infty[$ pour $g$ ✔. Note « mojibake : aucun » confirmée — le $\mathbb{R}$ de Q8 est rendu correctement (double barre) sur le scan, aucun glyphe cassé sur les deux pages. Filière SExp (شعبة العلوم التجريبية مسلك علوم الحياة والأرض ومسلك العلوم الفيزيائية — خيار فرنسية) et code NS 22F confirmés sur l'en-tête p. 1 (3 h, coef 7) et les bandeaux p. 3-4.
...
Statut: vérifié (corrigé sur re-lecture) — agent-vérificateur-adversarial, 2026-08-06 : source re-fetchée indépendamment (element/144505 → `course-438/upload-87124`, 4 pages re-dérivées ; **problème confiné à la seule p. 4 re-confirmé** — la p. 3 porte les Exercices 3 « Nombres complexes » et 4 « Calcul des probabilités ») ; diff caractère-par-caractère conforme au scan (Partie I 1→3, Partie II 1→5, bornes, exposants, $\ln\left(1-\frac{1}{xe^{-x}}\right)$ de Q2-b) — somme des barèmes = $1{,}25$ (Partie I) $+\ 6{,}75$ (Partie II) $=8$ pts = ligne « Problème · Etude de fonctions numériques et calcul intégral · 8 points » des composantes (p. 1) et bandeau « Problème (8 points) : » (p. 4). **Corrigé sur re-lecture** : (a) Q3-b, « Étudier » → « Etudier » — le scan porte l'E capital **sans accent** (zoom ×3), comme ailleurs dans la banque ; (b) figure, position de l'étiquette « $(\mathcal{C}_f)$ », mesurée sur le scan à $\approx (3\,;\,1)$ et posée **sous** la courbe (la courbe elle-même passe par $(3\,;\,1{,}16)$) — la valeur $(3\,;\,1{,}2)$ précédemment portée décrivait la courbe, pas l'étiquette. Mathématiques re-dérivées indépendamment : $f'(x)=\frac{1-x}{e^x-x}$ ✔ (dénominateur $>0$ par la Partie I, d'où croissance puis décroissance, maximum en $x=1$), $f(0)=1$ ✔, $f(x)=1-\ln(1-xe^{-x})\to 1$ en $+\infty$ ✔ (asymptote $y=1$), $f(x)/x\to 1$ et $f(x)-x\to-\infty$ en $-\infty$ ✔ (branche parabolique de direction $y=x$), zéro unique $\approx -0{,}78 \in\,]-1,0[$ ✔ (Q3-c), $f(x)=x \Leftrightarrow e^x-x=e$ donc $\alpha\simeq-2{,}65$ et $\beta\simeq1{,}42$ ✔ (Q4-a) et $e^{\alpha}-e^{\beta}=\alpha-\beta$ ✔ (Q4-b), aire de la Partie I $=\int_0^1(e^x-x)\,dx=e-\tfrac32$ ✔, $g$ croissante sur $I=]-\infty,1]$ vers $g(1)=2-\ln(e-1)$ ✔. **Jugement sur la figure : description jugée FIDÈLE au scan** après vérification au zoom point par point — cadre $x\in[-4,4]$ / $y\in[-3,4]$, droite de pente 1 par l'origine effectivement **non étiquetée**, allure confirmée (sous la droite et plus pentue au coin inférieur gauche, croisement en $\alpha$ dans le 3ᵉ quadrant, zéro entre $-1$ et $0$, passage par $(0\,;\,1)$, maximum $\approx1{,}5$ en $x=1$, second croisement entre $1$ et $2$, décroissance vers l'asymptote $y=1$) et corroborée par le calcul ; les deux marqueurs « (lecture à confirmer) » sont levés, seule la position de l'étiquette a dû être corrigée. Filière SExp (مسلك علوم الحياة والأرض ومسلك العلوم الفيزيائية — خيار فرنسية, sans le préfixe شعبة العلوم التجريبية en 2024) et code NS 22F confirmés sur l'en-tête p. 1 (3 h, coef 7) et le bandeau p. 4.
...
3. a) (0,5) Montrer que pour tout $x \in \mathbb{R}$ : $f'(x) = \dfrac{1 - x}{e^x - x}$
   b) (0,5) Etudier le signe de la fonction dérivée de $f$, puis déduire le tableau de variations de $f$ sur $\mathbb{R}$
...
4. La courbe $(\mathcal{C}_f)$ ci-contre est la représentation graphique de $f$ dans un repère orthonormé.
   ...
   > **Figure (description d'après le scan — re-jugée au zoom, 2026-08-06)** : repère orthonormé quadrillé, axe des abscisses gradué de $-4$ à $4$, axe des ordonnées gradué de $-3$ à $4$. Deux tracés : **(1)** une droite passant par l'origine, de pente $1$ — la droite d'équation $y = x$, non étiquetée sur le scan ; **(2)** la courbe $(\mathcal{C}_f)$, dont l'étiquette « $(\mathcal{C}_f)$ » est posée **juste sous la courbe**, vers $(3\,;\,1)$. La courbe monte depuis le coin inférieur gauche du cadre (où elle est **sous** la droite et plus pentue qu'elle), coupe la droite en un point du troisième quadrant d'abscisse $\approx -2{,}7$ *(lecture confirmée au zoom ; le calcul donne $\alpha \simeq -2{,}65$)*, coupe l'axe des abscisses en un point unique d'abscisse comprise entre $-1$ et $0$ *(calcul : $\approx -0{,}78$)*, passe par le point $(0\,;\,1)$, atteint un maximum d'ordonnée $\approx 1{,}5$ en $x = 1$ *(lecture confirmée ; le calcul donne $f(1) = 2 - \ln(e-1) \simeq 1{,}46$)*, recoupe la droite en un point d'abscisse comprise entre $1$ et $2$ *(calcul : $\beta \simeq 1{,}42$)*, puis décroît lentement vers l'ordonnée $1$ (cohérent avec l'asymptote horizontale $y = 1$ de la question 1-c) en restant **sous** la droite. Les deux intersections courbe–droite correspondent aux solutions $\alpha$ et $\beta$ de la question 4-a).
