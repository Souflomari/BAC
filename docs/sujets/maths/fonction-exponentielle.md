...
Statut: vérifié — agent-vérificateur-adversarial, 2026-08-06 : source re-fetchée indépendamment (element/109797 → `course-438/upload-80918`, 4 pages re-dérivées, problème sur p. 3-4) ; diff caractère-par-caractère de l'énoncé, des 9 questions et de chaque barème partiel conforme au scan — somme des barèmes = 7 pts = ligne « Problème · Etude d'une fonction numérique · 7 points » des composantes (p. 1) et bandeau « Problème : (7 points) » (p. 3). Mathématiques re-dérivées indépendamment : $f'(x)=-(e^{x-2}-1)^2$ ✔, $f''(x)=-2e^{x-2}(e^{x-2}-1)$ (changement de signe en $x=2$) ✔, $f(2)=2$ ✔ (le point $A(2,2)$ est bien sur $(C)$), $f(2+\ln 3)=2-\ln 3>0$ ✔ (cohérent avec la remarque de Q8-c) et $f(2+\ln 4)=\tfrac12-\ln 4<0$ ✔ (encadrement de $\alpha$ de Q6), $f(x)-(-x+\tfrac52)=-\tfrac12 e^{x-2}(e^{x-2}-4)$ ✔ (position relative de Q2-b). Glyphes cassés re-constatés un à un au zoom : $\mathbb{R}$ rendu « _ » (intro), « _ » (Q4-a), « Γ » (Q5), « □ » (Q8-a) — 4 occurrences, aucune autre — résolutions $\mathbb{R}$ confirmées par le contexte ($f$ continue strictement décroissante de $+\infty$ à $-\infty$, donc bijection de $\mathbb{R}$ sur $\mathbb{R}$) ; en Q7 le $\simeq$ est bien rendu par des barres cassées (« ln 2||0,7 et ln 3| 1,1 »). **Finding du repère manquant RE-CONFIRMÉ sur le scan** : sous la ligne Q7 (« dans le repère $(O,\vec i\,;\vec j)$ ci-dessous ») le bas de la p. 3 est un cadre rectangulaire strictement vide (zoom ×2 : ni quadrillage, ni axe, ni graduation) et la p. 4 s'arrête après Q8-c sur la marque de fin « ·/· » rouge — c'est bien une absence sur le scan, pas une omission de transcription. Filière SExp (شعبة العلوم التجريبية مسلك علوم الحياة والأرض ومسلك العلوم الفيزيائية — خيار فرنسية) et code NS 22F confirmés sur l'en-tête p. 1 (3 h, coef 7) et les bandeaux p. 3-4.
