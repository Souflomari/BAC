...
## La tâche
Écrire `class Explication(BacScene)` dans **`animations/scenes/maths/arithmetique/bk-2023-n-x4.py`**,
pour l'entrée **`bk-2023-n-x4`** de **`content/maths/arithmetique/bank.yaml`**
(**lignes 729 à 906**, barème 3 points).
...
python -c "import importlib.util,sys; sys.path.insert(0,'animations'); \
  spec=importlib.util.spec_from_file_location('s','animations/scenes/maths/arithmetique/bk-2023-n-x4.py'); \
  m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m); print('OK')"

# 1 — lint de scène
python scripts/scene-lint.py animations/scenes/maths/arithmetique/bk-2023-n-x4.py

# 2 — rendu brouillon (nb de sections == nb d'étapes)
cd animations && manim render scenes/maths/arithmetique/bk-2023-n-x4.py Explication -ql \
  --media_dir media/maths-arithmetique --save_sections && cd ..
ls animations/media/maths-arithmetique/videos/bk-2023-n-x4/480p15/sections/*.mp4 | wc -l
...
python scripts/bank-fidelity.py content/maths/arithmetique/bank.yaml bk-2023-n-x4 animations/scenes/maths/arithmetique/bk-2023-n-x4.py

# 5 — rendu final, puis statut
cd animations && manim render scenes/maths/arithmetique/bk-2023-n-x4.py Explication -qm \
  --media_dir media/maths-arithmetique --save_sections && cd ..
