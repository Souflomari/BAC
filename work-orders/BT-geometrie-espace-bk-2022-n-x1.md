# BT — bk-2022-n-x1 · geometrie-espace
...
## La tâche
Écrire `class Explication(BacScene)` dans **`animations/scenes/maths/geometrie-espace/bk-2022-n-x1.py`**,
pour l'entrée **`bk-2022-n-x1`** de **`content/maths/geometrie-espace/bank.yaml`**
(**lignes 168 à 303**, barème 3 points).
...
python -c "import importlib.util,sys; sys.path.insert(0,'animations'); \
  spec=importlib.util.spec_from_file_location('s','animations/scenes/maths/geometrie-espace/bk-2022-n-x1.py'); \
  m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m); print('OK')"

# 1 — lint de scène
python scripts/scene-lint.py animations/scenes/maths/geometrie-espace/bk-2022-n-x1.py

# 2 — rendu brouillon (nb de sections == nb d'étapes)
cd animations && manim render scenes/maths/geometrie-espace/bk-2022-n-x1.py Explication -ql \
  --media_dir media/maths-geometrie-espace --save_sections && cd ..
ls animations/media/maths-geometrie-espace/videos/bk-2022-n-x1/480p15/sections/*.mp4 | wc -l
...
python scripts/bank-fidelity.py content/maths/geometrie-espace/bank.yaml bk-2022-n-x1 animations/scenes/maths/geometrie-espace/bk-2022-n-x1.py

# 5 — rendu final, puis statut
cd animations && manim render scenes/maths/geometrie-espace/bk-2022-n-x1.py Explication -qm \
  --media_dir media/maths-geometrie-espace --save_sections && cd ..
...
- **Porte 1** :
```
— bk-2022-n-x1.py : 9 étapes, 0 repère(s)
...
- **Porte 3 — audit visuel & planches de contact** :
  1. Étape `02-intro-donnees` : cadre repère orthonormé direct, points $A(0,1,1)$, $B(1,2,0)$, $C(-1,1,2)$ et sphère $(S)$.
  2. Étape `03-q1-q2-plan-abc` : calcul des coordonnées des vecteurs $\vec{AB}=(1,1,-1)$, $\vec{AC}=(-1,0,1)$, produit vectoriel $\vec{AB}\wedge\vec{AC}=(1,0,1)=\vec{i}+\vec{k}$ et plan $(ABC) : x+z-1=0$.
  3. Étape `04-q3-sphere-equation` : équation de la sphère $(S) : (x-1)^2+(y-1)^2+(z-2)^2=2$.
  4. Étape `05-q4-tangence-plan-sphere` : distance $d(\Omega,(ABC))=\sqrt{2}=R \implies (ABC)$ tangent à $(S)$, et $\vec{\Omega A} = -1\cdot\vec{n} \implies A$ point de tangence.
  5. Étape `06-q5-droite-delta` : représentation paramétrique de $(\Delta)$ passant par $C(-1,1,2)$ et de direction $\vec{u}=(1,0,1)$.
  6. Étape `07-q6-tangence-droite-sphere` : distance point-droite $d(\Omega,(\Delta))=\sqrt{2}=R \implies (\Delta)$ tangente à $(S)$ en $D(0,1,3)$.
  7. Étape `08-q7-distance-point-droite` : produit scalaire $\vec{AC}\cdot(\vec{i}+\vec{k})=0 \implies \vec{AC}\perp\vec{u}_\Delta \implies d(A,(\Delta))=AC=\sqrt{2}$.
- **Porte 4** :
```
banque bank.yaml / bk-2022-n-x1 : 9 valeurs
scène  bk-2022-n-x1.py : 30 valeurs

· 21 valeur(s) propre(s) à la scène (illustrations,
  fenêtres de tracé — normal, à survoler) :
   5, 6, 7, 01, 02, 03, 04, 05, 06, 07, 08, 09, 22, 0.3, 0.5, 0.25, 0.75, 2022, 2026, 136586, 718096
