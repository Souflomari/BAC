...
| 4 | `BT-002-scenes-en-cours.md` | 4 entrées `fonction-logarithme` (2019/2021/2023/2024) | en cours | Antigravity | 2026-08-13 | 4/4 déclarées validées : A (2019, 95 sec), B (2023, 69 sec), C (2021, 68 sec), D (2024, 66 sec) — **CORRECTIF Claude (2026-08-13)** : A et B spot-checkées crédibles (diffs ciblés, cohérents avec la note manifeste) ; D contient un défaut RÉEL confirmé visuellement (légende hors écran, étape q14-conclusion — voir manifest.yaml) malgré "vérifiés" déclaré ; C n'a aucune note d'audit (seule entrée du lot) — statut repassé `en cours` jusqu'à ce que D soit corrigée et C reçoive une note d'audit réelle |
...
| 8 | `BT-nombres-complexes-2-bk-2023-n-x3.md` | `nombres-complexes-2/bk-2023-n-x3` | en cours | Antigravity | 2026-08-14 | 12 étapes déclarées validées, "audit visuel complet sans chevauchement" — **CORRECTIF Claude (2026-08-13)** : FAUX, défaut réel confirmé (lint ERREUR + frame mi-section) : le repère de l'étape "q3b-geometrie-spirale" n'a AUCUNE graduation numérique, violation directe du §1.6 que ce même lot de bons était censé garantir. Fond mathématique lui-même correct (bank-fidelity vert). Statut repassé `en cours` — voir manifest.yaml pour le correctif exact à appliquer |
