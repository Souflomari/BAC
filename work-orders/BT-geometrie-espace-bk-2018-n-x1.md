...
## La tâche
Écrire `class Explication(BacScene)` dans **`animations/scenes/maths/geometrie-espace/bk-2018-n-x1.py`**,
pour l'entrée **`bk-2018-n-x1`** de **`content/maths/geometrie-espace/bank.yaml`**
(**lignes 573 à 680**, barème 3 points).
...
python -c "import importlib.util,sys; sys.path.insert(0,'animations'); \
  spec=importlib.util.spec_from_file_location('s','animations/scenes/maths/geometrie-espace/bk-2018-n-x1.py'); \
  m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m); print('OK')"

# 1 — lint de scène
python scripts/scene-lint.py animations/scenes/maths/geometrie-espace/bk-2018-n-x1.py

# 2 — rendu brouillon (nb de sections == nb d'étapes)
cd animations && manim render scenes/maths/geometrie-espace/bk-2018-n-x1.py Explication -ql \
  --media_dir media/maths-geometrie-espace --save_sections && cd ..
ls animations/media/maths-geometrie-espace/videos/bk-2018-n-x1/480p15/sections/*.mp4 | wc -l
...
python scripts/bank-fidelity.py content/maths/geometrie-espace/bank.yaml bk-2018-n-x1 animations/scenes/maths/geometrie-espace/bk-2018-n-x1.py

# 5 — rendu final, puis statut
cd animations && manim render scenes/maths/geometrie-espace/bk-2018-n-x1.py Explication -qm \
  --media_dir media/maths-geometrie-espace --save_sections && cd ..
