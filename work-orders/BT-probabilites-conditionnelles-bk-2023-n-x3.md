# BT — bk-2023-n-x3 · probabilites-conditionnelles
...
## La tâche
Écrire `class Explication(BacScene)` dans **`animations/scenes/maths/probabilites-conditionnelles/bk-2023-n-x3.py`**,
pour l'entrée **`bk-2023-n-x3`** de **`content/maths/probabilites-conditionnelles/bank.yaml`**
(**lignes 52 à 180**, barème 3 points).
...
python -c "import importlib.util,sys; sys.path.insert(0,'animations'); \
  spec=importlib.util.spec_from_file_location('s','animations/scenes/maths/probabilites-conditionnelles/bk-2023-n-x3.py'); \
  m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m); print('OK')"

# 1 — lint de scène
python scripts/scene-lint.py animations/scenes/maths/probabilites-conditionnelles/bk-2023-n-x3.py

# 2 — rendu brouillon (nb de sections == nb d'étapes)
cd animations && manim render scenes/maths/probabilites-conditionnelles/bk-2023-n-x3.py Explication -ql \
  --media_dir media/maths-probabilites-conditionnelles --save_sections && cd ..
ls animations/media/maths-probabilites-conditionnelles/videos/bk-2023-n-x3/480p15/sections/*.mp4 | wc -l
...
python scripts/bank-fidelity.py content/maths/probabilites-conditionnelles/bank.yaml bk-2023-n-x3 animations/scenes/maths/probabilites-conditionnelles/bk-2023-n-x3.py

# 5 — rendu final, puis statut
cd animations && manim render scenes/maths/probabilites-conditionnelles/bk-2023-n-x3.py Explication -qm \
  --media_dir media/maths-probabilites-conditionnelles --save_sections && cd ..
...
- **Porte 1** :
```
— bk-2023-n-x3.py : 9 étapes, 0 repère(s)
...
- **Porte 3 — audit visuel & planches de contact** :
  1. Étape `02-intro-urnes` : représentation visuelle des urnes U1 et U2 avec flèche de transfert.
  2. Étape `03-q1a-event-a` : $p(A) = 3/6 = 1/2$.
  3. Étape `04-q1b-arbre-event-b` : $p(B) = (1/2)(2/6) + (1/6)(3/6) = 1/6 + 1/12 = 1/4$.
  4. Étape `05-q2-proba-conditionnelle` : $p(A/B) = \frac{p(A\cap B)}{p(B)} = \frac{1/6}{1/4} = 2/3$.
  5. Étape `06-q3a-variable-x-zero` : $X=ab$, $p(X=0) = p(a=0) = 2/6 = 1/3$.
  6. Étape `07-q3b-loi-de-x` : tableau de distribution de $X \in \{0, 1, 2, 4\}$, somme égale à 1.
  7. Étape `08-q3c-equiprobabilite-m-n` : $p(M) = p(N) = 1/3 \implies$ équiprobables.
- **Porte 4** :
```
banque bank.yaml / bk-2023-n-x3 : 7 valeurs
scène  bk-2023-n-x3.py : 28 valeurs

· 21 valeur(s) propre(s) à la scène (illustrations,
  fenêtres de tracé — normal, à survoler) :
   5, 01, 02, 03, 04, 05, 06, 07, 08, 09, 0.2, 0.3, 0.5, 1.1, 1.2, 2.1, 2.2, 0.25, 0.75, 2023, 718096
